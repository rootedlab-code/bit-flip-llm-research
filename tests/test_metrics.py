"""The measurement contract: the arithmetic is verified without running a model."""
...
def uniform_scorer(chunk: torch.Tensor) -> torch.Tensor:
    """A model that knows nothing: the same probability for every token."""
    return torch.zeros(chunk.numel(), VOCABULARY)


def oracle_scorer(chunk: torch.Tensor) -> torch.Tensor:
    """A model that knows the next token and gives it almost all the mass."""
...
def test_windows_use_the_widest_available_context():
    """From the second window on, every scored token has half a window of context.

    The last one is shorter because the sequence ends, not because context is missing.
    """
...
def test_a_stride_at_least_as_long_as_the_window_is_rejected():
    with pytest.raises(ValueError, match="not greater than stride"):
        list(sliding_windows(100, window=512, stride=512))


def test_uniform_model_has_perplexity_equal_to_the_vocabulary_size():
    """If every token is equally likely, per-token surprise is ln(V), so PPL = V."""
...
def test_windowing_does_not_change_the_measure_of_a_context_free_model():
    """A context-free model must give the same figure under any windowing."""
    tokens = torch.randint(0, VOCABULARY, (2000,))

    wide = perplexity(uniform_scorer, tokens, window=1024, stride=512)
    narrow = perplexity(uniform_scorer, tokens, window=128, stride=64)

    # The tolerance is that of the float32 softmax (~1e-7 relative), not a
    # convenience: below it one would be measuring rounding error, not the model.
...
def test_known_nll_gives_the_expected_perplexity():
    """A closed numeric check: two tokens, a known distribution, PPL worked by hand."""
    tokens = torch.tensor([0, 1])

    def biased(chunk):
        logits = torch.full((chunk.numel(), 2), 0.0)
        logits[0, 1] = math.log(3.0)  # p(token 1) = 3/4 after softmax over [0, ln 3]
        return logits
