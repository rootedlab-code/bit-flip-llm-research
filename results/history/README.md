...
| `e5-verdicts-spec-v2.csv` | per-probe verdicts, spec v2 | The diagnosis, and the numbers spec v3 was built against. `base/benign` — an aligned model on harmless questions, no flips and no ablation — reports the same 52% compliance and 48% indeterminate as the corner that failed, with the two classes at medians of 1,079 and 800 characters. Same keywords, same bar, same order of length: the procedure marker alone separated them, so structure was standing in for substance. The same table holds the constraint v3 had to respect — a quarter of `base/harmful` refusals run past 481 characters. |
